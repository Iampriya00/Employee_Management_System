...
function Office() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/employee/office_location")
      .then((response) => {
  ...
  const handleDeleteLocation = (id) => {
    ...
    axios
      .delete(`http://localhost:5000/employee/office_location/${id}`)
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:5000/employee/office_location", newLocation)
      .then((response) => {
        ...
        <>
          <ul className="list-group">
            {officeLocations.map((location) => (
              ...
              >
                {location.name} - Latitude: {location.latitude}, Longitude:{" "}
                {location.longitude}, Address: {location.address}
                <button
