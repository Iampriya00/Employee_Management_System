...
const ClockIn = () => {
    ...
    const fetchOfficeLocations = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/employee/office_location`
        );
    ...
    if (location === "home") {
      try {
        const response = await axios.post(
          `http://localhost:5000/employee/employee_clockin/${id}`,
          {
            work_from_type: "Home",
          }
        );
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          const isAtOfficeLocation = checkOfficeLocation(
            userLatitude,
            userLongitude
          );
  ...
  const handleClockOut = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/employee/employee_clockout/${id}`
      );
  ...
  return (
    <div>
      {!clockedIn && (
        ...
        >
          <i
            className="fs-5 bi bi-box-arrow-in-right"
      ...
      {clockedIn && (
        <div>
          ...
          >
            <i
              className="fs-5 bi bi-box-arrow-right"
              style={{ marginRight: "5px" }}
            ></i>{" "}
            Clock Out
          </button>
