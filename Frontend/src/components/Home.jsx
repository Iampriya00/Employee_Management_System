...
const Home = () => {
  ...
  const adminRecords = () => {
    axios
      .get("http://localhost:5000/auth/admin_records")
      .then((result) => {
  ...
  const adminCount = () => {
    axios
      .get("http://localhost:5000/auth/admin_count")
      .then((result) => {
        if (result.data.Status) {
  ...
  const employeeCount = () => {
    axios
      .get("http://localhost:5000/auth/employee_count")
      .then((result) => {
        if (result.data.Status) {
  ...
  const salaryCount = () => {
    axios
      .get("http://localhost:5000/auth/salary_count")
      .then((result) => {
        if (result.data.Status) {
  ...
  const handleEditSubmit = () => {
    axios
      .put(`http://localhost:5000/auth/edit_admin/${editingAdminId}`, {
        email: editedEmail,
      })
      .then((result) => {
  ...
  return (
    <div>
      <div className="p-3 d-flex flex-wrap justify-content-around mt-3">
        <div
          className="px-3 pt-2 pb-3 mb-3 HomeCard"
          style={{ backgroundColor: "#FFFAFA", width: "300px" }}
        >
        ...
        <div
          className="px-3 pt-2 pb-3 mb-3 HomeCard"
          style={{ backgroundColor: "#F7F9FF", width: "300px" }}
        >
          <div className="text-center pb-1">
            <h4>Employee</h4>
          </div>
        ...
        <div
          className="px-3 pt-2 pb-3 mb-3 HomeCard"
          style={{ backgroundColor: "#FFFDF8", width: "300px" }}
        >
          <div className="text-center pb-1">
            <h4>Salary</h4>
          </div>
